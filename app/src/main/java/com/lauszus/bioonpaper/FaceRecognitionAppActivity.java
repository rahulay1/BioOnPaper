...
public class FaceRecognitionAppActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    class VerifyThread extends Thread{
        @Override
        public void run() {
            ArrayList<byte[]> features =tinydb.getListBytes("features");
            ...
            labelProbArrayB= new byte[1][128];
    ...
    private void reSizeImage() {
        Bitmap photo=Bitmap.createBitmap(mRgba.width(),mRgba.height(), Bitmap.Config.ARGB_8888);;
        Utils.matToBitmap(mRgba, photo);
        Bitmap bitmap = getResizedBitmap(photo,  (int) (DIM_IMG_SIZE_X*photo.getWidth()/photo.getHeight()), DIM_IMG_SIZE_Y);
        System.out.println("Resize image size Width : " +bitmap.getWidth()+"Height :" +bitmap.getHeight());

        int cropSize = (int) (bitmap.getWidth()-bitmap.getHeight())/2;


        final Bitmap photoCropped = Bitmap.createBitmap( DIM_IMG_SIZE_X, DIM_IMG_SIZE_X,Bitmap.Config.ARGB_8888);
//        System.out.println("Source height :" +source.height());

        for (int x = 0; x < DIM_IMG_SIZE_X; x++) {
            for (int y = 0; y < DIM_IMG_SIZE_X; y++) {
                int ix = cropSize+DIM_IMG_SIZE_X- y;
                int iy = DIM_IMG_SIZE_X -x ;
                photoCropped.setPixel(x,y,bitmap.getPixel(ix-1,iy-1));
//                System.out.println("iy :" +iy);
    ...
    private MappedByteBuffer loadModelFile() throws IOException {
        AssetFileDescriptor fileDescriptor = this.getAssets().openFd("my_facenet.tflite");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(result != null){
            if(result.getContents()==null){
                ...
                integerStrings = result.getContents().split(" ");
                showToast("Distace is " +KNN.getDistance(integerStrings, labelProbArrayB[0]), Toast.LENGTH_LONG);
            }
        }
        else {
